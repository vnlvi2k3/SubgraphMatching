...
def concat_feat(f1, f2, n1, n2):
    chunk1 = torch.split(f1, n1)
    chunk2 = torch.split(f2, n2)
    h = []
    for i, j in zip(chunk1, chunk2):
        h.append(i)
        h.append(j)
    return torch.vstack(h)
...
class IEGMN_Layer(nn.Module):
    def __init__(self, orig_h_feats_dim, h_feats_dim, out_feats_dim, args):
        ...
        self.node_mlp = nn.Sequential(
            nn.Linear(
                orig_h_feats_dim + 2 * h_feats_dim + self.out_feats_dim, h_feats_dim
            ),
            nn.Dropout(dropout),
            get_non_lin(nonlin, leakyrelu_neg_slope),
    ...
    def forward(
        ...
        with graph.local_scope():
            ...
            x_rel_mag = torch.cat(
                [torch.exp(-x_rel_mag / sigma) for sigma in self.all_sigmas_dist],
                dim=-1,
            )

            if not self.use_dist_in_layers:
                x_rel_mag = x_rel_mag * 0.0
            ...
            n1 = bacth_sub_numnode
            n2 = graph.batch_num_nodes() - bacth_sub_numnode
            mask = get_mask(n1, n2, self.device)
            sub_feats, graph_feats = get_features(graph, bacth_sub_numnode)
            attention, aggr_cross1 = compute_cross_attention(
                self.att_mlp_Q(sub_feats),
                self.att_mlp_K(graph_feats),
                self.att_mlp_V(graph_feats),
                mask,
                self.cross_msgs,
            )  # (n x m), (n x d)

            _, aggr_cross2 = compute_cross_attention(
                self.att_mlp_Q(graph_feats),
                self.att_mlp_K(sub_feats),
                self.att_mlp_V(sub_feats),
                mask.transpose(0, 1),
                self.cross_msgs,
            )  # (m x n), (m x d)

            graph.ndata["aggr_cross_msg"] = concat_feat(
                aggr_cross1, aggr_cross2, n1.tolist(), n2.tolist()
            )
            ...
            x_final = (
                self.x_connection_init * original_coords
                + (1.0 - self.x_connection_init) * graph.ndata["x_now"]
                + graph.ndata["x_update"]
            )

            input_node_upd = torch.cat(
                (
                    self.node_norm(graph.ndata["feat"]),
            ...
            if self.h_feats_dim == self.out_feats_dim:
                node_upd = (
                    self.skip_weight_h * self.node_mlp(input_node_upd)
                    + (1.0 - self.skip_weight_h) * h_feats
                )
            else:
                node_upd = self.node_mlp(input_node_upd)

            node_upd = apply_final_h_layer_norm(
                graph, node_upd, self.final_h_layer_norm, self.final_h_layernorm_layer
            )
