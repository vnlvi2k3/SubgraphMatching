...
def parse_args():
    ...
    parser.add_argument(
        "--config", "-c", default="configs/geoshape.json", type=str, help="Config file"
    )
...
def gen_coordinates(size=1):
    x = list(np.random.uniform(-50.0, 50.0,size=(size,)))
    y = list(np.random.uniform(-50.0, 50.0,size=(size,)))
    z = list(np.random.uniform(-50.0, 50.0,size=(size,)))
    shuffle(x)
    shuffle(y)
    shuffle(z)
    return np.array(list(zip(x,y,z))).tolist()

def modify_coords(graph):
    modified_graph = graph.copy()
    num_modify = np.random.randint(1, len(modified_graph.nodes) +1)
    chosen_nodes = np.random.choice(modified_graph.nodes, num_modify, replace=False)
    for idx in chosen_nodes:
        modified_graph.nodes[idx]["coords"] = gen_coordinates(size=1)[0]
        # modified_graph.nodes[idx]["modified"] = True
        
    return modified_graph

def se3_transformer(alpha, beta, gamma, trans, x):
    
    rotate_X = np.array([[1,0,0],[0,np.cos(alpha),-np.sin(alpha)],[0,np.sin(alpha),np.cos(alpha)]])
    rotate_Y = np.array([[np.cos(beta),0,np.sin(beta)],[0,1,0],[-np.sin(beta),0,np.cos(beta)]])
    rotate_Z = np.array([[np.cos(gamma),-np.sin(gamma),0],[np.sin(gamma),np.cos(gamma),0],[0,0,1]])
    
    return rotate_Z@rotate_Y@rotate_X@x + trans

def add_labels(graph, NN, NE, coords):
    ...
    labelled_nodes = [
        (nodes[k], {"label": node_labels[k], "coords": coords[k] ,"color": "green"})
...
def generate_iso_subgraph(
    ...
    trans = np.random.uniform(-50.0,50.0,size=(3,))
    
    for node_idx in subgraph.nodes:
        subgraph.nodes[node_idx]["coords"] = se3_transformer(
            angles[0], angles[1], angles[2], trans, subgraph.nodes[node_idx]["coords"])
...
def add_random_nodes(
    ...
    for i in range(number_of_possible_nodes_to_add):
        ...
        node_coords = gen_coordinates(1)[0]
        added_nodes.append((node_id, {"label": node_label,"coords": node_coords, "modified": True}))
        node_id += 1
    ...
    **kwargs
    ...
    if type == 0:
        ...
        for node_idx in subgraph.nodes:
            subgraph.nodes[node_idx]["coords"] = se3_transformer(
                angles[0], angles[1], angles[2], trans, subgraph.nodes[node_idx]["coords"])
        subgraph = modify_coords(subgraph)
...
def generate_subgraphs(graph, number_subgraph_per_source, *args, **kwargs):
    ...
    for i in tqdm(range(number_subgraph_per_source)):
...
def generate_one_sample(
    ...
    coord, x, y = data
    coords = np.array(coord[1])
    edges = y[1].numpy()
    generated_pattern = nx.Graph()
    edgelist = [
        (edges[0][k], edges[1][k])
        for k in range(len(edges[0]))
    ]
    generated_pattern.add_edges_from(edgelist)

    labelled_pattern = add_labels(
        generated_pattern, number_label_node, number_label_edge, coords
    )
...
def generate_batch(dataset, start_idx, stop_idx, number_source, dataset_path, *args, **kwargs):
    for idx in range(start_idx, stop_idx):
        print("SAMPLE %d/%d" % (idx + 1, number_source))
        data_idx = dataset[idx]
        graph, iso_subgraphs, noniso_subgraphs = generate_one_sample(data_idx,
            *args, **kwargs)
        save_per_source(idx, graph, iso_subgraphs,
                        noniso_subgraphs, dataset_path)


def generate_dataset(dataset, dataset_path, is_continue, number_source, *args, **kwargs):
    print("Generating...")
    list_processes = []
        ...
        for start_idx, stop_idx in list_idx:
            list_processes.append(
                Process(
                    ...
                    args=(dataset, start_idx, stop_idx, number_source, dataset_path),
                    kwargs=kwargs,
                )
            )
...
def save_per_source(graph_id, H, iso_subgraphs, noniso_subgraphs, dataset_path):
    ...
    with open(source_graph_file, "w", encoding="utf-8") as file:
        file.write("t # {0}\n".format(graph_id))
        for node in H.nodes:
            c = H.nodes[node]["coords"]
            file.write("v {} {} {} {} {}\n".format(node, H.nodes[node]["label"], c[0], c[1], c[2]))
        for edge in H.edges:
            file.write(
                "e {} {} {}\n".format(
    ...
    for subgraph_id, S in enumerate(iso_subgraphs):
        ...
        for node_idx, node_emb in enumerate(list_nodes):
            c = S.nodes[node_emb]["coords"]
            isf.write("v {} {} {} {} {}\n".format(node_idx, S.nodes[node_emb]["label"], c[0], c[1], c[2]))
    ...
    for subgraph_id, S in enumerate(noniso_subgraphs):
        ...
        for node_idx, node_emb in enumerate(list_nodes):
            c = S.nodes[node_emb]["coords"]
            nisf.write("v {} {} {} {} {}\n".format(
                node_idx, S.nodes[node_emb]["label"], c[0], c[1], c[2]))
...
def main(config_file, is_continue):
    ...
    dataset = GeometricShapes(root='./raw/GeometricShapes', pre_transform=pre_transform)

    generate_dataset(dataset = dataset, dataset_path=dataset_path,
