...
def generate_one_sample(
    ...
    coords = np.array(traj.xyz[0])[:no_of_nodes]
...
def generate_batch(start_idx, stop_idx, number_source, dataset_path, *args, **kwargs):
    data_path = './raw/protein'
    for idx in range(start_idx, stop_idx):
        print("start: ", start_idx, "stop", stop_idx)
        print("SAMPLE %d/%d" % (idx + 1, number_source))

        ds_name = os.listdir(data_path)[idx]
        traj = md.load_pdb(os.path.join(data_path, ds_name))

        graph, iso_subgraphs, noniso_subgraphs = generate_one_sample(traj, 
            *args, **kwargs)
