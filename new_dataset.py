...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        H1_pre = onehot_encoding_node(m1, self.embedding_dim)
        sub_coordinates = []
        for id in m1.nodes:
            sub_coordinates.append(m1.nodes[id]["coords"])
        sub_coordinates = np.vstack(sub_coordinates)

        # Prepare source graph
        n2 = m2.number_of_nodes()
        adj2 = nx.to_numpy_array(m2) + np.eye(n2)
        H2_pre = onehot_encoding_node(m2, self.embedding_dim)
        graph_coordinates = []
        for id in m2.nodes:
            graph_coordinates.append(m2.nodes[id]["coords"])
        graph_coordinates = np.vstack(graph_coordinates)
        ...
        H1 = np.concatenate([H1_pre, np.zeros((n1, self.embedding_dim))], 1)
        H2 = np.concatenate([np.zeros((n2, self.embedding_dim)), H2_pre], 1)
        ...
        lig_graph = dgl.from_networkx(m1, node_attrs=['label', 'coords'])
        rec_graph = dgl.from_networkx(m2, node_attrs=['label', 'coords'])
        lig_graph.ndata['feat'] = torch.from_numpy(H1_pre).float()
        rec_graph.ndata['feat'] = torch.from_numpy(H2_pre).float()
...
def collate_fn(batch):
    ...
    P = np.zeros((len(batch), max_natoms , 3))
    Q = np.zeros((len(batch), max_natoms , 3))
    lig_list = []
    rec_list = []
    ...
    for i in range(len(batch)):
        ...
        Y[i] = batch[i]["Y"]
        V[i, :natom] = batch[i]["V"]
        lig_list.append(batch[i]["lig_graph"])
        rec_list.append(batch[i]["rec_graph"])

    
    lig_graphs = dgl.batch(lig_list)
    rec_graphs = dgl.batch(rec_list)
    ...
    return H, A1, A2, M, S, Y, V, lig_graphs, rec_graphs
