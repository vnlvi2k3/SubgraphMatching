...
from new_dataset import BaseDataset, collate_fn, UnderSampler
...
def main(args):
    ...
    for epoch in range(num_epochs):
        ...
        for sample in tqdm(train_dataloader):
            model.zero_grad()
            H, A1, A2, M, S, Y, V, lig_graphs, rec_graphs = sample
            H, A1, A2, M, S, Y, V, lig_graphs, rec_graphs = (
                H.to(device),
                A1.to(device),
                A2.to(device),
                M.to(device),
                ...
                Y.to(device),
                V.to(device),
                lig_graphs.to(device),
                rec_graphs.to(device),
            )

            # Train neural network
            pred, ligs_keypts, recs_keypts = model(
                X=(H, A1, A2, V), lig_graph=lig_graphs, rec_graphs=rec_graphs, attn_masking=(M, S), training=True
            )
        ...
        for sample in tqdm(test_dataloader):
            ...
            rmsd = []
            for i in range(len(ligs_keypts)):
                rmsd.append(kabsch_rmsd(ligs_keypts, recs_keypts))
